...
if ($is_query_run = mysqli_query($conn, $query)) {
    ...
    while ($row = mysqli_fetch_assoc($is_query_run)) {
        ...
        echo '<input class="delete-popup-checkbox" name="delete-list[]" type="checkbox" value="' . $cat . $row["id"] . '"><a href="admin-detail.php?cat=art&id=' . $row["id"] . '"><p class="cat-title">' . $row["title"] . '</p><p class="cat-location">' . $row["location"] . '</p><p class="cat-mdy">' . $mdy . '</p><p class="cat-hm">' . $hm . '</p></a></li>';
        echo "</div>";
        echo "</div>";
    }
    /* free result set */
...
if (isset($_POST["submit"])) {
    ...
    if (empty($title) || strlen($title) > 25) {
        ...
        $artSql = "INSERT INTO arts (id, category, title, location, date, Image_dir,description)
    ...
    if (!empty($_POST['delete-list'])) {
        try {
            ...
            foreach ($_POST['delete-list'] as $selected) {
                ...
                $stmt = $conn->prepare('DELETE FROM arts WHERE id = :uniqueId');
