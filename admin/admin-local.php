...
<head>
    ...
    <title>Vancouver's Best Ticket Booking : Admin Locals</title>
    <!-- Firebase setting -->
                ...
                <form name="add-event" id="add-form"   method="post" enctype="multipart/form-data">
                    <div class="admin-cat-add-input">
                        ...
                        <select id="select-category" name="selectCat">
                            ...
                            <option value="locals" selected>Locals</option>
                        </select>
                    </div>
...
if ($is_query_run = mysqli_query($conn, $query)) {
    ...
    while ($row = mysqli_fetch_assoc($is_query_run)) {
        ...
        echo '<input class="delete-popup-checkbox" name="delete-list[]" type="checkbox" value="' . $cat . $row["id"] . '"><a href="admin-detail.php?cat=loc&id=' . $row["id"] . '"><p class="cat-title">' . $row["title"] . '</p><p class="cat-location">' . $row["location"] . '</p><p class="cat-mdy">' . $mdy . '</p><p class="cat-hm">' . $hm . '</p></a></li>';
        echo "</div>";
        echo "</div>";
    }
    /* free result set */
...
?>
    ...
    <footer class="admin-login-footer"></footer>
</body>
...
if (isset($_POST["submit"])) {
    ...
    if (empty($title) || strlen($title) > 30) {
        ...
        $result = "Error : Invalid Title Input, Title must be 1~20 letters";
    } elseif (empty($loc) || strlen($loc) > 30) {
        //Location validation : 1~20 letters
        $result = "Error : Invalid Location Input, Location must be 1~20 letters";
    } elseif ($date < $currentDate) {
        //Date validation : no later than curret date
        $result = "Error : Invalid Date Input, Date must be no later than current date ";
    } else {
        $artSql = "INSERT INTO locals (id, category, title, location, date, Image_dir,description)
    ...
    if (!empty($_POST['delete-list'])) {
        try {
            ...
            foreach ($_POST['delete-list'] as $selected) {
                ...
                $stmt = $conn->prepare('DELETE FROM locals WHERE id = :uniqueId');
                //$stmt->bindParam(':tableName', $tableN);
                $stmt->bindParam(':uniqueId', $uniqId);
                $stmt->execute();
                ("Location: admin-local.php");
