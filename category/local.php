    ...
    <div>
        ...
        <div id="catPanel" class="panel">
            ...
            <div id="cat-sort-result">
                <?php
                /* fetch only 4 pieces of data from 'locals' */
                $query = 'SELECT * FROM `locals` ORDER BY id LIMIT 4';
                if ($is_query_run = mysqli_query($conn, $query)) {
                    ...
                    while ($row = mysqli_fetch_assoc($is_query_run)) {
                        ...
                        echo '<img src="' . $row["Image_dir"] . '" alt=""><li><a href="detail.php?cat=loc&id=' . $row["id"] . '"><p class="cat-title">' . $row["title"] . '</p><p class="cat-location">' . $row["location"] . '</p><p class="cat-mdy">' . $mdy . '</p><p class="cat-hm">' . $hm . '</p></a></li>';
                        echo '</div>';
                    }
                    echo '</ul>';
                    /* free result set */
...
<script>
    ...
    function sortBy(str){
        ...
        xmlhttp.open("GET", "list.php?q=l" + str + "&from=" + from + "&to=" + to, true);
        xmlhttp.send();
    }
